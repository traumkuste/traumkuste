...
function renderLogs() {
  ...
  if (hasLogs) {
    ...
    html += G.logs.map(function(l) {
      return '<div class="log-card"><div class="log-time">' + l.time + '</div><div class="log-text">' + l.text + '</div></div>';
    }).join('');
  }
  el.innerHTML = html;
}
...
function loadData(callback) {
  fetch('data.json')
    ...
    .catch(function(e) {
      ...
      LCOLOR={'空中都市':'#E5D0E3','湖':'#9297C4','庭':'#ADB993','浜辺':'#FED18C','海':'#90F1EF','深海':'#2d4a6e'};
      LTEXT={'空中都市':'#3a1f3a','湖':'#ffffff','庭':'#2a3a1e','浜辺':'#5c3e00','海':'#0a3a3a','深海':'#b8d4f0'};
