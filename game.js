...
function genEquip(depth, lck) {
  ...
  var item = {
    ...
    type: '装備(仲間)',
    icon: base.icon,
    desc: base.jp + ' ' + base.stat.toUpperCase() + '+' + base.vals[tier] + '・' + adj.forms[tier] + '（' + (EG.tierJp[tier] || '') + adj.jp + '）— ' + effLabel,
    stat: stat,
  ...
  if (iconEl) {
    var partyIcons = dungState.party.map(function(c) {
      ...
      return '<span title="' + c.word + '" style="' + (dead ? 'opacity:.25;filter:grayscale(1)' : '') + '">'
        + '<img src="' + spriteURL(c.word, c.article, c.layer, c.level) + '" style="width:20px;height:20px;image-rendering:pixelated;vertical-align:middle" alt=""></span>';
    }).join('');
